...
export const FASTLANE_SERVICES = [
	...
	{
		id: 7,
		name: 'Generator/Inverter Rental',
		price: '₹699',
		time: 'Scheduled',
		icon: '🔋',
		tag: 'RENTAL',
	...
	{
		id: 11,
		name: '2-Wheeler Service @ Doorstep',
		price: '₹499',
		time: 'Scheduled',
		icon: '🏍️',
		tag: '',
...
const extractNumericPrice = (priceStr) => {
	if (!priceStr) return 500; // Default fallback
...
export default function BookingWizard({ isOpen, onClose, onSuccess, user, initialService = null }) {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const numericPrice = extractNumericPrice(selectedService.price);
			...
			await api.post('/request', {
				...
				estimatedPrice: numericPrice, // Send as numeric value
			});
			onSuccess?.();
			onClose();
		} catch (err) {
		...
		<div className="cd-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
			<div className="cd-modal">
				...
				<div className="cd-welcome-banner">
					...
					<span>
						Welcome to <strong>Fastlane</strong> — Proceed with a single tap
					</span>
				...
				<div className="cd-steps">
					{STEPS.map((label, idx) => (
						<div
							key={label}
							className={`cd-step ${idx < step ? 'done' : ''} ${idx === step ? 'active' : ''}`}
						>
							...
							{idx < 3 && (
								<div className={`cd-step-line ${idx < step ? 'done' : ''}`} />
							)}
						</div>
					))}
				</div>
							...
							{FASTLANE_SERVICES.map((svc) => (
								...
								>
									<div className="cd-svc-top">
										...
										{svc.tag && (
											<span className={`cd-svc-tag cd-tag-${svc.tag}`}>
												{svc.tag}
											</span>
										)}
									</div>
				...
				{step === 3 && (
					<div className="cd-step-body">
						...
						<div className="cd-order-summary">
							...
							<div className="cd-order-note">
								⚠️ Extra charges apply beyond 5 km radius
							</div>
						</div>
						<div className="cd-form-group">
							<label>
